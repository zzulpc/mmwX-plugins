...
单个任务的排队与执行预算合计最多 118 秒；超时后会清理进程并回传结果。两段分别限时：

| 段 | 额度 | 说明 |
|---|---|---|
| 排队等待 | 60 秒 | 到期退出等待并回报失败，不再等待前序任务结束 |
| 实际执行 | 58 秒 | 内核就绪 15s + sing-box 校验 10s + 出口 IP 5s + 延迟 5s + 下载准备 10s + 吞吐窗口 8s + 余量 5s |

**执行预算只从任务真正拿到执行权之后才开始走**，排队耗时不会压缩这 58 秒。
内核定位、准备和测速均计入执行时限；首次自动下载内核也不会额外延长任务期限。
主控断线会取消排队和正在执行的任务。0.2.6 之前两段混用同一个 38 秒时钟，
...
设置 `MMWX_PROBE_ALLOW_PRIVATE=1` 放开；只有 `1` 生效，`0`、`false`、空值或其它值均保持限制。
IPv6 地址中的接口标识（如 `%eth0`）不会改变地址分类，无法解析的地址也会被拒绝。
...
- **修复任务时间预算不闭合**：执行预算改为拿到串行执行权之后才起算，并与排队预算分开计量；
  排队超时或断线可取消等待，回包后释放任务槽；行为测试验证排队不会挤压执行预算。
...
- **修复延迟探测不看 HTTP 状态码**：落地门户或错误页返回的 200 曾被当成有效延迟，
  仅测延迟模式下还会直接回报成功；现在只接受 204 空响应，不跟随跳转，读取失败的样本也会丢弃。
- 可达性探测默认拒绝内网与保留地址，避免被当作家庭内网端口扫描器；
  仅 `MMWX_PROBE_ALLOW_PRIVATE=1` 可放开，修复 IPv6 接口标识绕过限制的问题。
- 地址限制测试只使用回环监听和直接判定，不再拨号到非回环目标。
